...
import com.psp.data.AlumnoRepository
        ...
        lifecycleScope.launch {
            authToken?.let { token ->
                repository.getAlumnoById(token, id).fold(
                    onSuccess = { alumno ->
                        Log.d("alumno", "Alumno encontrado: $alumno")
                    },
                    onFailure = { error ->
                        Log.e("alumno", "Error alumno $id: ${error.message}")
                    }
                )
            } ?: Log.e("MainActivity", "No hay token. Haz login")
        }
    }
        ...
        lifecycleScope.launch {
            authToken?.let { token ->
                repository.getAlumnosByCurso(token, curso).fold(
                    ...
                    onFailure = { error ->
                        Log.e("Curso", " curso $curso: ${error.message}")
                    }
                )
            } ?: Log.e("MainActivity", "No hay token. Haz login")
        }
                ...
                onSuccess = { token ->
                    ...
                    alumnosResult.fold(
                        onSuccess = { alumnos ->
                            ...
                            messageState.value = "Alumnos : ${alumnos.size}"
                        },
                        onFailure = { error ->
